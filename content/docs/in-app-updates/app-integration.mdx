...
This example shows a complete React component that loads update status, starts immediate or flexible update flows, tracks download progress, and falls back to the store page when needed.
...
export function InAppUpdatePanel() {
  ...
  const loadStatus = useCallback(async () => {
    ...
    try {
      ...
      if (error instanceof InAppUpdatesError) {
        ...
        setMessage('Something went wrong while checking for updates.');
      }
    } finally {
      setLoading(false);
    }
  ...
  const startImmediate = useCallback(async () => {
    ...
    try {
      ...
      if (error instanceof InAppUpdatesError) {
        ...
        setMessage('Something went wrong while starting the update.');
      }
    } finally {
      setLoading(false);
    }
  ...
  const startFlexible = useCallback(async () => {
        ...
        void completeFlexibleUpdate()
          ...
          .catch((error) => {
            if (error instanceof InAppUpdatesError) {
              ...
              setMessage('Something went wrong while completing the update.');
            }
          });
      }
    });
    ...
    try {
      ...
      if (error instanceof InAppUpdatesError) {
        ...
        setMessage('Something went wrong while starting the download.');
      }
    } finally {
      setLoading(false);
    }
  ...
  const openStore = useCallback(async () => {
    ...
    try {
      ...
      if (error instanceof InAppUpdatesError) {
        ...
        setMessage('Something went wrong while opening the store.');
      }
    } finally {
      setLoading(false);
    }
  ...
  return (
    <View>
      ...
      <Text>Supported: {status?.supported ? 'yes' : 'no'}</Text>
      <Text>
...
- Register the install-state listener before calling `startFlexibleUpdate()` so you receive download progress from the start.
- Clean up the listener on unmount to avoid stale callbacks.
- Complete a flexible update only when the install status is `downloaded`.
- Use the store page fallback when an in-app update flow is not available.
- If you refresh status separately and want to show a manual Complete update button, guard it with `canCompleteFlexibleUpdate(status)`.
